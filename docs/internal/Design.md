...
其中：
	...
	1. 客户端请求由一个独立的协程处理。该协程负责监听端口，接受对端的请求，并且分发到对应的工作协程
...
[配置文件示例](../config_example.txt)

##### 配置文件的热加载
配置文件的热加载工作由负责监听的协程完成, 每次监听协程接受客户端连接并新建工作线程进行处理之后，监听协程会检查配置文件，发现更改进行热加载。  

其工作流程如下图:

![](./ConfigReloadFlow.png)
 
#### 连接处理模块

##### 后端连接池。
后端连接池以cluster为单位，其中又划分为读连接池和写连接池，每个前端user对每个cluster有独立的连接池。  

后端连接处理完请求满足下列条件后才可放入连接池:
* 连接处于可用状态(不可用状态如前端异常关闭，相应后端连接关闭等情况)
* 连接处于绑定状态(如事务过程中、prepare等)
* 连接池中数量未超过限制

每个cluster使用一个管理协程负责管理管理其后端连接池，协程完成的工作包括：
* 预建后端连接
* 后端探活
* 检查后端连接，关闭超时连接
* 后端连接的failover
