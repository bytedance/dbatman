...
func (node *Create) Format(buf *TrackedBuffer) {
	switch node.Obj {
	case AST_DATABASE, AST_FUNCTION:
		buf.Myprintf("create %s%s %s", node.Obj, node.NotExistsOpt, node.ID)
	}
}
...
func (node *CreateTable) Format(buf *TrackedBuffer) {
	// buf.Myprintf("create table%s %v %s", node.NotExistsOpt, node.Table, node.ID)
}
...
func (node *Use) Format(buf *TrackedBuffer) {
	buf.Myprintf("%s %s", node.Action, node.ID)
}
...
type ColValues []ColTuple
type RowValues []RowTuple

func (node ColValues) Format(buf *TrackedBuffer) {
	prefix := "values "
	for _, n := range node {
		buf.Myprintf("%s%v", prefix, n)
		prefix = ", "
...
func (node RowValues) Format(buf *TrackedBuffer) {
	prefix := "values "
	for _, n := range node {
		buf.Myprintf("%s%v", prefix, n)
		prefix = ", "
...
const (
	AST_EXISTS     = " if exists"
	AST_NOT_EXISTS = " if not exists"
)
...
func (node *Drop) Format(buf *TrackedBuffer) {
	buf.Myprintf("drop %s%s %s", node.Key, node.ExistsOpt, node.ID)
}
