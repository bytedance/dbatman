...
func (n *Node) checkMaster() {
	...
	if db == nil {
		AppLog.Notice("no master avaliable")
		return
	}

	if err := db.Ping(); err != nil {
		AppLog.Warn("%s ping master %s error %s", n, db.Addr(), err.Error())
	} else {
		n.lastMasterPing = time.Now().Unix()
		return
	}
	...
	if int64(n.downAfterNoAlive) > 0 && time.Now().Unix()-n.lastMasterPing > int64(n.downAfterNoAlive) {
		AppLog.Warn("%s down master db %s", n, n.master.Addr())
