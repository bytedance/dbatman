...
func TestConn_DeleteTable(t *testing.T) {
	...
	if _, err := c.Execute("drop table if exists go_proxy_test_conn"); err != nil {
		t.Fatal(err)
	}
}

func TestConn_CreateTable(t *testing.T) {
	s := `CREATE TABLE IF NOT EXISTS go_proxy_test_conn (
...
func TestConn_Insert(t *testing.T) {
	s := `insert into go_proxy_test_conn (id, str, f, e) values(1, "a", 3.14, "test1")`
...
func TestConn_Select(t *testing.T) {
	s := `select str, f, e from go_proxy_test_conn where id = 1`
...
func TestConn_Escape(t *testing.T) {
	...
	e := `""''\abc`
	s := fmt.Sprintf(`insert into go_proxy_test_conn (id, str) values(5, "%s")`,
		Escape(e))
	...
	s = `select str from go_proxy_test_conn where id = ?`
